...
const EditExam: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAssessment = async (id: string) => {
      try {
        ...
        handleInputChange("courseId", response.data.data.courseId._id);
        handleInputChange(
          "numberOfQuestions",
  ...
  const handleContinue = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (currentStep === 0) {
      try {
        ...
        if (response.data.data._id !== examData._id) {
      ...
      setCurrentStep(-1); // Go back to summary
    }
  };
  ...
  const handleFinish = async () => {
    const updateExam: AssessmentEntity = {
    ...
    try {
      const response = await updateAssessmentApi(id!, updateExam);
...
const Summary: React.FC<SummaryProps> = ({
  ...
  const matchingCourse = courses.find(
    (course) => course._id === examData.courseId
  );
  return (
    <Card className="w-full max-w-md">
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          <p>
            <strong>Number of Questions:</strong> {examData.questions.length}
          </p>
          <p>
